...
test('parses operators', () => {
  const operators = '+\\^-*';
  const expectedTokens = [...operators].map((value) => ({
    type: 'operator',
    value,
  }));
...
test('parses punctuation', () => {
  const punctuation = '({)},';
  const expectedTokens = [...punctuation].map((value) => ({
    type: 'punctuation',
    value,
  }));
