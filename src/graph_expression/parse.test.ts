...
const call = (name: string, args: Expression[]) => ({ type: 'call', name, args })
const binary = (operator: Operator, left: Expression, right: Expression): Expression => ({ type: 'binaryOperator', operator, left, right })
...
test('parses call', () => {
  ...
  expect(parse(lex('fib(1)'))).toEqual({'ok': true, value: call('fib', [number]) })
  expect(parse(lex('fib(1, abc)'))).toEqual({'ok': true, value: call('fib', [number, name]) })
})

test('parses binary operator', () => {
  expect(parse(lex('abc + 1'))).toEqual({'ok': true, value: binary('+', name, number)})
  expect(parse(lex('abc + 1 + {}'))).toEqual({'ok': true, value: binary('+', binary('+', name, number), set([]))})
})

test('binary operator precedence', () => {
  const two: Expression = { type: 'number', value: 2 }
  expect(parse(lex('1 * abc - 2'))).toEqual({'ok': true, value: binary('-', binary('*', number, name), two)})
  expect(parse(lex('1 + abc * 2'))).toEqual({'ok': true, value: binary('+', number, binary('*', name, two))})
  expect(parse(lex('1 ^ abc \\ 2'))).toEqual({'ok': true, value: binary('\\', binary('^', number, name), two)})
})
...
test('parses parenthesis', () => {
  ...
  expect(parse(lex('(1, (1, abc))'))).toEqual({'ok': true, value: tuple([number, tuple([number, name])])})
})
...
test('parses set', () => {
  ...
  expect(parse(lex('{abc, 1}'))).toEqual({'ok': true, value: set([name, number]) })
  expect(parse(lex('{abc, {1}}'))).toEqual({'ok': true, value: set([name, set([number])]) })
})
...
test('parsing bigger expression', () => {
  expect(parse(lex('Bipartite(2,3) ^ 2 + ({1, 2}, {(1, 2)}) + (Peterson)'))).toEqual(
    {
      "ok": true, "value": {
        "left": {
          "left": {
          ...
          "right": {
            "type": "tuple",
            "values": [
              { "type": "set", "values": [{ "type": "number", "value": 1 }, { "type": "number", "value": 2 }] },
              {
                "type": "set",
                "values": [{
                  "type": "tuple",
