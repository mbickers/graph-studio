...
export function parse(tokens: Token[]): Result<Expression, string> {
  ...
  const tryWrapWithBinaryAbovePrecedence = (
    left: Expression,
    ...
    if (
      next?.type === 'operator' &&
      precedence[next.value] > currentPrecedence
    ) {
      take();
      ...
      const right = tryWrapWithBinaryAbovePrecedence(
        rightFirstAtom.value,
        precedence[next.value],
      );
      if (!right.ok) {
        return right;
      }
